...
import { VeniceClient, type VeniceConfig, type VeniceGenerateRequest } from './venice-client.js';
...
export type InferenceProvider = 'venice' | 'bittensor' | 'local';
...
export class SecureInferenceEngine {
  ...
  async generate(request: SecureInferenceRequest): Promise<SecureInferenceResult> {
    ...
    // ── Tier 3: Local model ───────────────────────────────────────────────
    tried.push('local');
    const localResult = await this.local.generate({
      prompt: request.prompt,
      model: request.model,
      maxTokens: request.maxTokens,
      temperature: request.temperature,
    ...
    logInferenceCost({
      ...
      cost_usd: 0,
      duration_ms: localResult.responseTime,

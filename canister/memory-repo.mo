...
actor MemoryRepo {
  ...
  /// A single ThoughtForm entry snapshotted inside a commit.
  public type ThoughtFormEntry = {
    id        : Text;
    content   : Text;
    tags      : [Text];
    timestamp : Int;
  ...
  public shared(msg) func store_thoughtform(
    ...
    let existingForms : [ThoughtFormEntry] = switch (parentHash) {
      case null { [] };
      case (?ph) {
    ...
    let updatedForms = Array.append<ThoughtFormEntry>(existingForms, [thoughtform]);

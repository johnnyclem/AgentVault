...
describe('Scenario: Full fallback chain', () => {
  ...
  it('Given Venice and Bittensor fail, local model is tried last', async () => {
    vi.stubGlobal('fetch', async (url: string) => {
      ...
      if (url.includes('localhost:11434') && url.includes('/api/chat')) {
        return new Response(
          JSON.stringify({ message: { role: 'assistant', content: 'Local response' }, model: 'llama3' }),
          { status: 200, headers: { 'Content-Type': 'application/json' } },
        );
      }
  ...
  it('Inference cost is logged for the chosen provider (no key material in log)', async () => {
    vi.stubGlobal('fetch', async (url: string) => {
      ...
      if (url.includes('localhost:11434/api/chat')) {
        return new Response(
          JSON.stringify({ message: { role: 'assistant', content: 'Local ok' }, model: 'llama3' }),
          { status: 200, headers: { 'Content-Type': 'application/json' } },
        );
      }
